...
public class MGChargeActivity extends AppCompatActivity implements Observer {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((CheckBox)findViewById(R.id.onCheck)).setOnCheckedChangeListener((compoundButton, isChecked) -> {
            ...
            mgChargeApplication.setShouldSuperviseDevice( isChecked || ((CheckBox)findViewById(R.id.offCheck)).isChecked() );
            setOnGroup();
        });
        ((CheckBox)findViewById(R.id.offCheck)).setOnCheckedChangeListener((compoundButton, isChecked) -> {
            ...
            mgChargeApplication.setShouldSuperviseDevice( isChecked || ((CheckBox)findViewById(R.id.onCheck)).isChecked() );
            setOffGroup();
        });
    ...
    private void createTimeOCL(TextView textView, String prefName){
        textView.setOnClickListener(v -> {
            ...
            mTimePicker = new TimePickerDialog(MGChargeActivity.this, (timePicker, selectedHour, selectedMinute) -> {
                preferences.edit().putString(prefName,  String.format(Locale.ENGLISH,"%02d:%02d", selectedHour,selectedMinute)).apply();
    ...
    void exitApplication(){
        Log.i(MGChargeApplication.TAG, NameUtil.context());
        mgChargeApplication.setShouldSuperviseDevice(false);
    ...
    protected void onResume() {
        ...
        mgChargeApplication.mgChargeActivityRunning = true;
        mgChargeApplication.appObservable.addObserver(this);
        setHead();
        mgChargeApplication.wakeupDeviceHandler();
    }
    ...
    protected void onPause() {
        super.onPause();
        Log.i(MGChargeApplication.TAG, NameUtil.context());
        mgChargeApplication.appObservable.deleteObserver(this);
        mgChargeApplication.mgChargeActivityRunning = false;
    }
    ...
    private void setHead(){
        if (mgChargeApplication.selectedDevice != null){
            ((TextView)findViewById(R.id.device)).setText(mgChargeApplication.selectedDevice.getName());
            String text;
            if (mgChargeApplication.isConnected()){
                text = getResources().getString(R.string.connected) + String.format(Locale.ENGLISH," - %s - %.1fW",mgChargeApplication.isOn?"on":"off",mgChargeApplication.powerMeter);
            } else {
    ...
    private void setDirectHeadGroup(){
        boolean connected = mgChargeApplication.connected;
        ((CheckBox)findViewById(R.id.directCheck)).setChecked(preferences.getBoolean("directCheck", false));
        setEnabledDependencies(connected, findViewById(R.id.directHead), findViewById(R.id.directCheck), findViewById(R.id.directDescription), findViewById(R.id.directSwitch) );
        findViewById(R.id.directCheck).setEnabled(connected);
        findViewById(R.id.directHead).setEnabled(connected);
        if (connected){
            ((SwitchCompat)findViewById(R.id.directSwitch)).setChecked(mgChargeApplication.isOn);
        }
    }
    ...
    private void setDirectGroup(){
        ...
        Log.i(MGChargeApplication.TAG, NameUtil.context()+ " mgChargeApplication.isOn="+mgChargeApplication.isOn);
        ((SwitchCompat)findViewById(R.id.directSwitch)).setChecked(mgChargeApplication.isOn);
    }
    ...
    public void setDirectSwitch(boolean setChecked){
        mgChargeApplication.shouldBOn = setChecked;
        mgChargeApplication.wakeupDeviceHandler();
